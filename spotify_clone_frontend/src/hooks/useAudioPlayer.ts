...
import type { Track } from "../services/tracksService";

/**
 * PUBLIC_INTERFACE
 * useAudioPlayer provides controls and state for an audio playlist using HTMLAudioElement.
 */
export function useAudioPlayer(initialTracks: Track[] = [], initialIndex = 0) {
  ...
  const currentTrack: Track | undefined = useMemo(() => tracks[currentIndex], [tracks, currentIndex]);

  const loadIndex = useCallback(async (index: number, autoPlay = false) => {
  ...
  const loadTracks = useCallback((newTracks: Track[], startIndex = 0, autoPlay = false) => {
    setTracks(newTracks);
    if (newTracks.length) {
  ...
  const setVolumePercent = useCallback((v: number) => {
    // v in 0..1
